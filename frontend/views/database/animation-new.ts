function createAnimationNewVue(selectName: string, location: {mode: string, tab: string, id: number | string, route(hash: string, params?: Object)}) {
    ...
    let vm = new Vue({
        ...
        computed: {
            ...
            relationChoices() {
                let ret = []
                for(let i = 0; i < RELATION_NAME.length; ++i) ret[i] = {relation: RELATION_SEQUENCE[i], name: RELATION_NAME[i]}
                return ret
            },
            relationNameEnums() {
                let ret = {}
        ...
        methods: {
            load() {
                ...
                function doSomething(isStaff: boolean) {
                    if(isStaff) {
                        vm.updateTagList()
                        vm.updateStaffList()
                        vm.eventComeTo(() => {vm.ui.loading = false})
                    }else{
            ...
            eventComeTo(callback: (ok: boolean) => void = null) {
                if(location.mode === 'new') {
                    ...
                    if(backend != null && backendId != null && backendId === this.editId) {
                        ...
                        this.query((ok) => {if(callback != null) callback(ok)})
                    }
                }
            },
            eventLeaveFrom() {
            ...
            refreshEditor() {
                ...
                this.data.staffSupervisors = mapArray(backend.staff_supervisors, (staffId) => {return {id: staffId, name: staffInfo[staffId].name}})
                this.data.staffCompanies = mapArray(backend.staff_companies, (staffId) => {return {id: staffId, name: staffInfo[staffId].name}})
                this.data.originalWorkAuthors = mapArray(backend.original_work_authors, (staffId) => {return {id: staffId, name: staffInfo[staffId].name}})
                ...
                this.data.links = mapArray(backend.links, (link: string) => {return {value: link}})
                                ...
                                for(let r of RELATION_SEQUENCE) {
                                    if(oldValue[r] && newValue[r]) {
                                        ...
                                        for(let i = 0; i < oldValue[r].length; ++i) {
                                            function getValue(value) {return typeof value === 'object' ? value.id : value}
            ...
            searchStaff() {
                if(this.ui.staffEditor.search.trim()) {
                    let search = this.ui.staffEditor.search.trim().split(' ')
                    let result = []
                ...
                if(searchText) {
                    ...
                    client.database.animations.list({search: searchText, ordering: '-update_time'}, (ok, s, d) => {
                        if(ok) {
                            ...
                            this.ui.relationEditor.showResult = mapArray(d, (item) => {return item.id !== editId ? {
            ...
            moveSearchItem(index: number, toTab: string, toListIndex: number | null) {
                ...
                goalList.splice(toListIndex != null || goalList.length, 0, {id: item.id, title: item.title, cover: item.cover})
            },
            //link管理
            addNewLink() {
    ...
    $('#animation-new #trash-modal').modal({
        detachable: false,
        duration: 200,
        onApprove() {
            vm.trash()
